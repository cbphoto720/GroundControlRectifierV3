...
function PickControlPointsV1()
    ...
    function LOADUTC(~, ~)
        ...
        [fileNameUTC, pathNameUTC, fileNameLLZ, pathNameLLZ, timeZone] = SelectLoadFiles();
        ...
        if ~isempty(fileNameUTC)
            ...
            [wholeSecondFrameNum, wholeFrameDateTime, lastFrame] = FindNearestWholeSecondFrame(fileNameUTC, pathNameUTC, timeZone);
    ...
    function [utcFname, utcPname, surveyFname, surveyPname, timeZone] = SelectLoadFiles()
        
        hLoadDialog = dialog('Units', 'character',...
                             'Position', [ssW/2-30 ssH/2-15 100 20],...
                             'Name', 'Load Raw Data',...
                             'WindowStyle', 'modal',...
        ...
        function SelectUTCFile(~, ~)
            ...
            if ~isempty(utcFname) && ~isempty(surveyFname)
                hOKBtn.Enable = 'on'; 
            end
    ...
    function SAVE(hObject, ~)
        ...
        if figdata.ChangeFlag == 1 && hObject == hfig
            ...
            elseif strcmp(saveWarn, 'Yes')
                ...
                if filterIndex ~= 0
                    save([pathNameSave fileNameSave], 'points', 'rectParams', '-mat');
                    figdata.ChangeFlag = 0;
                    delete(hfig);
                end
            end
        ...
        elseif hObject == saveBtn
            ...
            if isempty(figdata.SavePath)
                ...
                save(figdata.SavePath, 'points', 'rectParams', '-mat');
                figdata.ChangeFlag = 0;
            end
            saveBtn.Enable = 'on';
    ...
    function [wholeSecFrameNumbers, wholeFrameDateTime, lastFrame] = FindNearestWholeSecondFrame(fName, pName, tmz)
        ...
        remainder = mod(length(minWholeSecs),30);
        minWholeSecs = minWholeSecs(1:end-remainder); % only keep a multipe of 30 to make reshape work
        tempReshape = reshape(minWholeSecs,[30,length(minWholeSecs)/30]);
    ...
    function [latGPS, lonGPS, XGPS, YGPS, ZGPS, dateTimeGPS] = LoadSurvey(fName, pName)
        ...
        fid = fopen([pName fName],'r');
        surveyData = textscan(fid,'%*d %*d %*d %f %f %*f %*f %f %f %f %*f %f %f %f %f %*d %*d %*f %*f',...
        ...
        YGPS = surveyData{4};
        ZGPS = surveyData{5};
        dateGPS = surveyData{6};
        yearGPS = floor(dateGPS./10000);
        monthGPS = floor((dateGPS-(yearGPS.*10000))./100);
        dayGPS = dateGPS - floor(dateGPS./100).*100;
        timeGPS = surveyData{7}.*10^4 + surveyData{8}.*10^2 + surveyData{9};
        hourGPS = surveyData{7};
        minGPS = surveyData{8};
        secGPS = surveyData{9};
    
        [~, sortInd]= sort(dateGPS.*10^6 + timeGPS, 'ascend');
